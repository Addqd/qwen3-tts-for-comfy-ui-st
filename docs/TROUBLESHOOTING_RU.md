...
## BAT сообщает `ComfyUI project PID file is unavailable`

Это означает, что `http://127.0.0.1:8188` уже отвечает, но текущая ComfyUI была запущена не проектным launcher либо её `runtime/comfyui.json` утрачен. Без проверенного PID launcher намеренно не завершает и не «усыновляет» такой процесс.

Закройте существующее окно ComfyUI Python и снова запустите `start-tts-and-comfyui.bat`. Начиная с исправленной версии launcher выполняет эту проверку **до** запуска backend, поэтому после ошибки на `8020` не остаётся новый процесс. Если backend был оставлен старой версией launcher, один раз выполните `.\stop.ps1`.
