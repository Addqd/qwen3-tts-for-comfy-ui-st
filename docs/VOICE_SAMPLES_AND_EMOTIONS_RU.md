...
Самый простой способ — двойной щелчок по `start-tts-and-comfyui.bat`. Окно BAT должно оставаться открытым. После закрытия отдельного окна ComfyUI watcher остановит только backend, который был запущен этим BAT.
