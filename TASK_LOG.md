...
## 2026-08-10 — Подготовка Russian Base adaptation на ветке merged PR #7

- Подготовлен отдельный, не интегрированный в runtime pipeline для `Qwen/Qwen3-TTS-12Hz-0.6B-Base` и `Qwen/Qwen3-TTS-12Hz-1.7B-Base`. Исходные snapshots в `model_cache` используются только для чтения; будущие результаты разделены в `trained_models/qwen3-tts-0.6b-russian-tuned` и `trained_models/qwen3-tts-1.7b-russian-tuned`.
- Официальный upstream SFT проверен как single-speaker/full-parameter путь с переводом результата в `custom_voice`; для сохранения произвольного Base zero-shot/ICL cloning выбран selective LoRA только на 196 main-talker attention/MLP projections. Speaker encoder, text/codec embeddings, text projection, codec head, весь code predictor/sub-talker и speech tokenizer заморожены; итоговая конфигурация обязана оставаться `tts_model_type=base`.
- Единственный источник данных зафиксирован как `lab260/golos_balalaika` revision `7b41d4d0030b22ee3d10d6bdc79a43a4a428cecf`. Train selection целится в 7200 секунд с seed `2602026`, rarity/diversity ranking и ограничением по shard, а held-out берётся только из официального `test` split. Training text использует обычное поле `punct` с fallback `rover`; accent с `+` и phoneme annotations не подаются модели как текст.
- Поскольку dataset `speaker_id` не подтверждён как глобальная межрепличная идентичность, pipeline не создаёт фиктивные same-speaker пары. Используется self-conditioned reference каждой target-реплики через замороженный speaker encoder, сохраняя разнообразное voice conditioning без подмены Base на фиксированный голос.
- Малый реальный data smoke выполнен только на официальном `test` split без tokenization/training: 53 eligible rows; детерминированно выбраны 30 held-out rows, 120.41 секунды, 20 selection groups; все references self-conditioned, обычный training text не содержит accent `+`.
- Один foreground PowerShell runner устанавливает только pinned training dependency в `.venv`, проверяет CUDA/архитектуры, готовит dataset/manifests/codes и затем последовательно запускает ровно одну эпоху 0.6B и 1.7B. Поддержаны partial code manifests, LoRA/optimizer/scaler resume checkpoints, JSONL metrics, отдельные `SUCCESS.json`/`FAILED.json` и ненулевой exit при сбое; следующий размер после сбоя не запускается.
- Статические проверки: обе локальные Base-конфигурации совпали с ожидаемыми размерами, для каждой найдено 196 разрешённых LoRA modules; PEFT 0.20.0 meta-smoke создал 392 только-LoRA trainable tensors. Новый unit suite: `6 passed`; compileall, PowerShell parser и `pip check` успешны. Полный suite воспроизвёл 5 прежних runtime-settings-dependent failures в `test_emotion_router.py` при локальном `runtime/tts-settings.json`; 107 остальных тестов прошли, training-файлы к этим сбоям не относятся.
- Реальное кодирование train subset и GPU training не запускались. ComfyUI-ноды и все workflow JSON, включая canonical `voice_profile_from_wav_ru.json`, не изменялись и не создавались.
- Перед публикацией GitHub connector подтвердил, что PR #7 уже merged/closed (`9bde608c`). По прямому ограничению пользователя новый PR не создаётся и merge не выполняется; follow-up сохраняется на той же head-ветке `agent/backend-quality-voice-lab`.
- Создан локальный контрольный commit. Публикация той же ветки заблокирована внешней авторизацией: сохранённый `gh` token признан invalid, HTTPS push вернул `SEC_E_NO_CREDENTIALS`, а GitHub App имеет read-доступ, но write API вернул 403. Новый PR не создавался; после повторного `gh auth login` нужен только обычный push текущей ветки.
