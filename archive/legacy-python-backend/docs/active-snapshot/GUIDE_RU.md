...
Регрессионные примеры `Она пришла`, `Это она`, `Он и она` проверяют, что backend chunking не создаёт искусственную границу внутри слова. Это устраняет backend-side разрывы, но не может детерминированно исправить любой стохастический фонетический дефект Qwen. Тяжёлый ASR и недоказанный automatic retry намеренно не добавлены.
