...
Project node package `0.4.1` показывает выбор модели непосредственно в **Qwen TTS Server** и **Qwen TTS Synthesize**. Тяжёлая модель по-прежнему находится только в backend.

![Qwen TTS Server после обновления](images/comfyui/comfyui-workflow-completed.png)
...
- **Inherit Server model** — новый default Synthesize; использует model из Server. Старые serialized explicit values продолжают разрешаться без миграции.
...
![Заполненный Pronunciation overrides](images/comfyui/comfyui-stable-russian.png)

Документация использует пять уникальных реальных кадров. Совпадающий кадр переиспользуется для нескольких пояснений вместо хранения бинарных дубликатов под разными именами.
